...
/* For every 4 pixels there are 6 bytes and there are 8 extra start bytes
 * and 5 extra end */
#define GOODIX511_RAW_FRAME_SIZE 8 + (GOODIX511_HEIGHT * GOODIX511_SCAN_WIDTH) / 4 * 6 + 5
...
static void
read_otp_callback(FpDevice *dev, guint8 *data, guint16 len, gpointer ssm, GError *err)
{
  if (err)
    {
...
activate_run_state(FpiSsm *ssm, FpDevice *dev)
{
  switch (fpi_ssm_get_cur_state(ssm))
    ...
    case ACTIVATE_SET_MCU_IDLE:
      goodix_send_mcu_switch_to_idle_mode(dev, 20, goodixtls5xx_check_none, ssm);
      break;
    ...
    case ACTIVATE_UPLOAD_MCU_CONFIG:
      goodix_send_upload_config_mcu(dev, (guint8 *) goodix_511_config,
...
fpi_device_goodixtls511_class_init(FpiDeviceGoodixTls511Class *class)
{
  ...
  /* 20 enrollment stages: validated by SIGFM benchmark — reducing to 15
   * raises FRR by ~12 pp; reducing to 10 raises FRR by ~23 pp; increasing
   * to 30 shows no improvement.  The Windows driver uses 20 as well. */
