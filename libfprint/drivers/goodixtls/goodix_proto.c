...
void
goodix_encode_pack(guint8 flags, guint8 *payload, guint16 payload_len, gboolean pad_data,
...
goodix_encode_protocol(guint8 cmd, const guint8 *payload, guint16 payload_len,
                       gboolean calc_checksum, gboolean pad_data, guint8 **data,
                       guint32 *data_len)
{
  ...
  *data = g_malloc0(*data_len);
  ...
  if (calc_checksum)
    (*data)[sizeof(GoodixProtocol) + payload_len]
        = 0xaa - goodix_calc_checksum(*data, sizeof(GoodixProtocol) + payload_len);
  else
    (*data)[sizeof(GoodixProtocol) + payload_len] = GOODIX_NULL_CHECKSUM;
}

gboolean
goodix_decode_pack(guint8 *data, guint32 data_len, guint8 *flags, guint8 **payload,
                   guint16 *payload_len, gboolean *valid_checksum)
{
  ...
  *payload_len = length;
  *valid_checksum
      = goodix_calc_checksum(data, sizeof(GoodixPack)) == data[sizeof(GoodixPack)];

  return TRUE;
}

gboolean
goodix_decode_protocol(guint8 *data, guint32 data_len, guint8 *cmd, guint8 **payload,
                       guint16 *payload_len, gboolean *valid_checksum,
                       gboolean *valid_null_checksum)
{
  ...
  *payload_len = length;
  *valid_checksum = 0xaa - goodix_calc_checksum(data, sizeof(GoodixProtocol) + length)
                    == data[sizeof(GoodixProtocol) + length];
  *valid_null_checksum = GOODIX_NULL_CHECKSUM == data[sizeof(GoodixProtocol) + length];
