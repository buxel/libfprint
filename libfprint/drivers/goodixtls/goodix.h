...
/**
 * @brief Start the read loop that lets us get data from the device asynchronously. Should
 * be called upon device activation
 *
 * @param dev
 */
void
...
/**
 * @brief Tell the device we want an image from it. The response will be TLS encrypted so
 * you probably don't want to call this from your driver, checkout goodix_tls_read_image()
 * if you want an image from the device
 *
 * @param dev
 * @param callback
 * @param user_data
 ...
 * @param callback called once the user has presented their finger or with an error. Note
 * may be called at once if the mode bytes are wrong
 * @param user_data
 */
void
...
/**
 * @brief Ask the device what firmware version it is running. Response is null-terminated
 * string
 *
 * @param dev
 * @param callback
 ...
 * @note You probably don't need to call this from your driver directly, checkout the
 * goodix_tls_* functions
 *
 * @param dev
 * @param callback
 * @param user_data
...
/**
 * @brief Reset the internal state of the communication with the device. Use this e.g. on
 * device deactivation (where it may be reactivated again in future)
 *
 * @param dev
 */
void
