...
goodix_send_nop(FpDevice *dev, GoodixNoneCallback callback, gpointer user_data)
{
  ...
  if (callback)
    {
      cb_info = g_malloc(sizeof(GoodixCallbackInfo));

      cb_info->callback = G_CALLBACK(callback);
      cb_info->user_data = user_data;

      goodix_send_protocol(dev, GOODIX_CMD_NOP, (guint8 *)&payload, sizeof(payload), NULL,
                           FALSE, GOODIX_TIMEOUT, FALSE, goodix_receive_none, cb_info);
      goodix_receive_done(dev, NULL, 0, NULL);
      return;
    }

  goodix_send_protocol(dev, GOODIX_CMD_NOP, (guint8 *)&payload, sizeof(payload), NULL,
                       FALSE, GOODIX_TIMEOUT, FALSE, NULL, NULL);
  goodix_receive_done(dev, NULL, 0, NULL);
}
...
goodix_send_mcu_get_image(FpDevice *dev, GoodixImageCallback callback, gpointer user_data)
{
  ...
  goodix_send_protocol(dev, GOODIX_CMD_MCU_GET_IMAGE, (guint8 *)&payload, sizeof(payload),
                       NULL, TRUE, GOODIX_TIMEOUT, TRUE, NULL, NULL);
}
  ...
  if (callback)
    {
      ...
      goodix_send_protocol(dev, GOODIX_CMD_NAV_0, (guint8 *)&payload, sizeof(payload),
                           NULL, TRUE, GOODIX_TIMEOUT, TRUE, goodix_receive_default,
                           cb_info);
      return;
    }

  goodix_send_protocol(dev, GOODIX_CMD_NAV_0, (guint8 *)&payload, sizeof(payload), NULL,
                       TRUE, GOODIX_TIMEOUT, TRUE, NULL, NULL);
}
                                    ...
                                    GoodixNoneCallback callback, gpointer user_data)
{
  ...
  goodix_send_protocol(dev, GOODIX_CMD_MCU_SWITCH_TO_IDLE_MODE, (guint8 *)&payload,
                       sizeof(payload), NULL, TRUE, GOODIX_TIMEOUT, FALSE, NULL, NULL);
}
                                 ...
                                 GoodixDefaultCallback callback, gpointer user_data)
{
  ...
  goodix_send_protocol(dev, GOODIX_CMD_READ_SENSOR_REGISTER, (guint8 *)&payload,
                       sizeof(payload), NULL, TRUE, GOODIX_TIMEOUT, TRUE, NULL, NULL);
}
  ...
  if (callback)
    {
      ...
      goodix_send_protocol(dev, GOODIX_CMD_UPLOAD_CONFIG_MCU, config, length, free_func,
                           TRUE, GOODIX_TIMEOUT, TRUE, goodix_receive_success, cb_info);
      return;
    }

  goodix_send_protocol(dev, GOODIX_CMD_UPLOAD_CONFIG_MCU, config, length, free_func, TRUE,
                                         ...
                                         gpointer user_data)
{
  ...
  goodix_send_protocol(dev, GOODIX_CMD_SET_POWERDOWN_SCAN_FREQUENCY, (guint8 *)&payload,
                       sizeof(payload), NULL, TRUE, GOODIX_TIMEOUT, TRUE, NULL, NULL);
}
                        ...
                        gpointer user_data)
{
  ...
  goodix_send_protocol(dev, GOODIX_CMD_ENABLE_CHIP, (guint8 *)&payload, sizeof(payload),
                       NULL, TRUE, GOODIX_TIMEOUT, FALSE, NULL, NULL);
}
  ...
  if (callback)
    {
      ...
      goodix_send_protocol(dev, GOODIX_CMD_RESET, (guint8 *)&payload, sizeof(payload),
                           NULL, TRUE, GOODIX_TIMEOUT, TRUE, goodix_receive_reset,
                           cb_info);
      return;
    }

  goodix_send_protocol(dev, GOODIX_CMD_RESET, (guint8 *)&payload, sizeof(payload), NULL,
                       TRUE, GOODIX_TIMEOUT, TRUE, NULL, NULL);
}
  ...
  if (callback)
    {
      ...
      goodix_send_protocol(dev, GOODIX_CMD_REQUEST_TLS_CONNECTION, (guint8 *)&payload,
                           sizeof(payload), NULL, TRUE, 0, TRUE, goodix_receive_default,
                           cb_info);
      return;
    }
                                         ...
                                         gpointer user_data)
{
  ...
  if (callback)
    {
      ...
      cb_info->callback = G_CALLBACK(callback);
      cb_info->user_data = user_data;
      // special case: timeout needs to be at least 10ms and it will always timeout for
      // some reason todo: work out why it always times out for this but not the python
      // driver
  ...
  goodix_send_protocol(dev, GOODIX_CMD_TLS_SUCCESSFULLY_ESTABLISHED, (guint8 *)&payload,
                       sizeof(payload), NULL, TRUE, 10, FALSE, NULL, NULL);
}
...
goodix_tls_init(FpDevice *dev, GoodixNoneCallback callback, gpointer user_data)
{
  ...
  priv->tls_hop = g_malloc(sizeof(GoodixTlsServer));

  if (!priv->tls_ready_callback)
    priv->tls_ready_callback = g_malloc(sizeof(GoodixCallbackInfo));
