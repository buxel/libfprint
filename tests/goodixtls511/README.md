...
|-------------------|--------------|-----------------------------------------|
| `custom.py`       | **Ready**    | Python GI test script (open/enroll/verify/close) |
| `device`          | **TODO**     | umockdev sysfs recording of the USB device |
| `custom.pcapng`   | **TODO**     | Recorded USB traffic for pcap replay    |
...
deterministic TLS in the driver (PSK-only, fixed server random) so that
pcap replay produces identical handshake bytes.
...
1. Verify the test replay works:
   ```bash
   FP_DEVICE_EMULATION=1 python3 tests/umockdev-test.py tests/goodixtls511/
   ```

2. Uncomment `'goodixtls511'` in `tests/meson.build`
...
> **Note:** If the sensor firmware rejects PSK-only (requires DHE-PSK),
> a different approach will be needed — see `goodixtls.c` comments.
